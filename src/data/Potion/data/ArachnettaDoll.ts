...
export class ArachnettaDoll extends Talisman {
  get talismanKind() {
    return PotionKind.ArachnettaDoll;
  }
  ...
  SetPassiveEffect() {
    this.data.resource.goldCap.RegisterMultiplier(new MultiplierInfo(MultiplierKind.TalismanPassive, MultiplierType.Mul, () => this.passiveEffectValue));
  }

  SetEffect(heroKind: HeroKind, equipNum: Function) {
    return this.data.skill.baseAttackPoisonChance[heroKind].RegisterMultiplier(
      new MultiplierInfo(
