...
export class OctobaddieDoll extends Talisman {
  get talismanKind() {
    return PotionKind.OctobaddieDoll;
  }
  ...
  SetPassiveEffect() {
    this.data.resource.slimeCoinEfficiency.RegisterMultiplier(new MultiplierInfo(MultiplierKind.Talisman, MultiplierType.Mul, () => this.passiveEffectValue));
  }

  SetEffect(heroKind: HeroKind, equipNum: Function) {
    return this.data.skill.skillEffectRangeMultiplier[heroKind].RegisterMultiplier(
      new MultiplierInfo(
