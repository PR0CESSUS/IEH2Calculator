...
export class ChallengeHandicap_OctobaddieLv400 extends CHALLENGE_HANDICAPPED {
  get kind() {
    return ChallengeKind.HCOctobaddie400;
  }

  ClassExclusiveRewardString() {
    return [
      `Armored Fury + ${Util.percent(0.1 * this.data.challenge.permanentRewardMultiplier.Value())}`,
      `Warded Fury + ${Util.percent(0.1 * this.data.challenge.permanentRewardMultiplier.Value())}`,
      `Gold Cap + ${Util.percent(0.1 * this.data.challenge.permanentRewardMultiplier.Value())}`,
      `Stone Gain + ${Util.percent(1 * this.data.challenge.permanentRewardMultiplier.Value())}`,
      `Crystal Gain + ${Util.percent(1 * this.data.challenge.permanentRewardMultiplier.Value())}`,
      `Leaf Gain + ${Util.percent(1 * this.data.challenge.permanentRewardMultiplier.Value())}`,
    ];
  }
  ...
  SetReward() {
    this.data.stats.SetEffectStats(
      ...
      new MultiplierInfo(
        ...
        MultiplierType.Mul,
        () => 0.1 * this.data.challenge.permanentRewardMultiplier.Value(),
