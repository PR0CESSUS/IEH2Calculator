...
export class MonsterPet {
  ...
  get passiveEffectKind2() {
    return MonsterParameter.petPassiveEffectKinds2[this.species][this.color];
  }
  ...
  SetEffect() {
    switch (this.passiveEffectKind) {
      ...
      case PetPassiveEffectKind.SkillProfGain:
        this.data.stats.SetEffectStats(Stats.SkillProficiencyGain, new MultiplierInfo(MultiplierKind.Pet, MultiplierType.Mul, () => this.effectValue));
        break;
    ...
    switch (this.passiveEffectKind2) {
      case PetPassiveEffectKind2.GoldGain2:
        this.data.stats.GoldGain().RegisterMultiplier(new MultiplierInfo(MultiplierKind.Pet, MultiplierType.Mul, () => this.effectValue2));
        break;
      case PetPassiveEffectKind2.TamingPointGain2:
        this.data.stats.SetEffectStats(Stats.TamingPointGain, new MultiplierInfo(MultiplierKind.Pet, MultiplierType.Mul, () => this.effectValue2));
        break;
      case PetPassiveEffectKind2.RareMatChanceForBlueSlime2:
        this.data.monster.colorMaterialDropChance.RegisterMultiplier(new MultiplierInfo(MultiplierKind.Pet, MultiplierType.Mul, () => this.effectValue2));
        break;
      case PetPassiveEffectKind2.RareMatChance2:
        this.data.monster.colorMaterialDropChance.RegisterMultiplier(new MultiplierInfo(MultiplierKind.Pet, MultiplierType.Mul, () => this.effectValue2));
        break;
      case PetPassiveEffectKind2.ChestPortalOrbNumber2:
        // this.data.area.chestPortalOrbNumber.RegisterMultiplier(new MultiplierInfo(MultiplierKind.Pet, MultiplierType.Add, () => Math.floor(this.effectValue2)));
        break;
    }
  }
  get tempeffectValue() {
    return MonsterParameter.PetPassiveEffectValue(this.passiveEffectKind, this.rank) * (1.0 + this.loyalty / 100.0) * this.data.monster.petPassiveEffectMultiplier.Value();
  }
  get effectValue2() {
    return Math.max(this.tempeffectValue - this.maxEffectValue, 0.0) * MonsterParameter.PetPassiveEffectValue2(this.passiveEffectKind2);
  }
  ...
  PassiveEffectString() {
    if (MonsterParameter.petPassiveEffectKinds2[this.species][this.color])
      return `${Localization.MonsterPetPassiveEffectString(this)} (${Localization.MonsterPetPassiveEffectString2(this)})`;
