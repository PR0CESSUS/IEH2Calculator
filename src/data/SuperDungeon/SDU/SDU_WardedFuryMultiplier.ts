...
export class SDU_WardedFuryMultiplier extends SuperDungeonUpgrade {
  get kind() {return SuperDungeonUpgradeKind.WardedFuryMultiplier;}

  
  Cost(level) {return (2000 + 2000 * level) * Math.pow(2.0, level / 10.0);}

  SetEffect() {return globalThis.data.stats.SetEffectStats(Stats.WardedFury, new MultiplierInfo(MultiplierKind.SDUpgrade, MultiplierType.Mul, (() => this.EffectValue())));}
