...
export class SDU_PetMagicalCriticalChanceMultiplier extends SuperDungeonUpgrade {
  get kind() {return SuperDungeonUpgradeKind.PetMagicalCriticalChanceMul;}

  
  Cost(level) {return (1000 + 1000 * level) * Math.pow(2.0, level / 10.0);}

  SetEffect() {return globalThis.data.stats.SetEffectStats(Stats.PetMagCritChance, new MultiplierInfo(MultiplierKind.SDUpgrade, MultiplierType.Mul, (() => this.EffectValue())));}
