...
export class SDU_LoyaltyPointGainMul extends SuperDungeonUpgrade {
  get kind() {return SuperDungeonUpgradeKind.LoyaltyPointGainMul;}

  
  Cost(level) {return (5000 + 5000 * level) * Math.pow(2.0, level / 10.0);}

  SetEffect() {return globalThis.data.stats.SetEffectLoyalityPointGain(new MultiplierInfo(MultiplierKind.SDUpgrade, MultiplierType.Mul, (() => this.EffectValue())));}
