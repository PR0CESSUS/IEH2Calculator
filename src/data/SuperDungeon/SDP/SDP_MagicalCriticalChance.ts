...
export class SDP_MagicalCriticalChance extends SuperDungeonPowerup {
  get kind() {return SuperDungeonPowerupKind.MagicalCriticalChance;}
  ...
  SetEffect() {return globalThis.data.stats.HeroStats(this.heroKind, Stats.MagCritChance).RegisterMultiplier(new MultiplierInfo(MultiplierKind.DungeonItem, MultiplierType.After, (() => this.EffectValue() * this.level)));}

  SetGlobalEffect() {globalThis.data.stats.SetEffectStats(Stats.MagCritChance, new MultiplierInfo(MultiplierKind.DungeonItemPermanent, MultiplierType.After, (() => this.PermanentEffectValue()), (() => this.isUnlocked)));}
