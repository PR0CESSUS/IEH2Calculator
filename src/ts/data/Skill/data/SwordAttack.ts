...
export class SwordAttack extends SKILL {
  ...
  Damage() {
    let num = this.Damage();
    if (this.classSkill.stances[1].isActive)
      num *= 1.0 + this.classSkill.stances[1].effectValueBuff;
    return num;
  }

  
  DebuffChance() {return globalThis.data.skill.baseAttackPoisonChance[this.heroKind].Value() > 0.0 ? globalThis.data.skill.baseAttackPoisonChance[this.heroKind].Value() : this.classSkill.stances[3].effectValueBuff;}
