...
export class ChargeSwing extends SKILL {
  ...
  get debuff() {return Debuff.MatkDown;}

  Range() {return this.level >= 100 ? this.Range() + 30 * globalThis.data.skill.skillRangeMultiplier[this.heroKind].Value() : this.Range();}

  EffectRange() {
    if (this.level >= 150)
      return this.EffectRange() + 170 * globalThis.data.skill.skillEffectRangeMultiplier[this.heroKind].Value();
  ...
  Interval() {return this.level >= 200 ? this.Interval() * 0.75 : this.Interval();}
}
