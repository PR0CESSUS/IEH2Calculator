...
export class BATTLE {
  ...
  get fire() {
    return globalThis.data.monster.GlobalInformationChallengeBoss(this.color).Fire();
  }
  get ice() {
    return globalThis.data.monster.GlobalInformationChallengeBoss(this.color).Ice();
  }
  get thunder() {
  ...
  DamageModifier(value) {
    return this.battleCtrl.isSuperDungeon
      ? value *
          this.battleCtrl.superDungeonCtrl.damageMultiplier.Value() *
  ...
  Attacked(
    ...
    this.slayerOilDamage = this.isSlayerOil
      ? this.tempTotalDamage * globalThis.data.stats.ElementSlayerDamage(this.battleCtrl.heroKind, element).Value()
      : 0.0;
